...
public class Leaf {
    ...
    /** Corresponding table aliases used */
    protected List<String> extendedAliasList;

    /** tracks whether or not to outer join tables */
    protected List<Boolean> extendedOuter;

    /** tracks outer level */
    protected List<Integer> extendedOuterLevel;
    ...
    public Leaf() {
        ...
        extendedTableList = new ArrayList<String>();
        extendedAliasList = new ArrayList<String>();
        extendedOuter = new ArrayList<Boolean>();
        extendedOuterLevel = new ArrayList<Integer>();
    ...
    public void determineSelectStatement(QueryPlan aQueryPlan) {
        ...
        ArrayList<String> tableRedoList = new ArrayList<String>();
        ArrayList<String> aliasRedoList = new ArrayList<String>();

        for (int i = 0; i < joinConditions.size(); i++) {
            joinCondList.add(joinConditions.get(i).getJoinString());
        }

        for (int i = 0; i < extendedTableList.size(); i++) {
            String sTableName = extendedTableList.get(i);

            tableRedoList.add(sTableName);
            ...
            // {
            aliasRedoList.add(extendedAliasList.get(i));
            // }
            // else
            // {
            // aliasRedoList.addElement ("");
            // }
        }
        ...
        while (tableRedoList.size() > 0 && lastSize != tableRedoList.size()) {
            lastSize = tableRedoList.size();

            for (int i = 0; i < tableRedoList.size(); i++) {
                String sTableName = tableRedoList.get(i);
                String sAlias = aliasRedoList.get(i);
                ...
                if (sbFromOn.length() == 0) {
                    ...
                    tableRedoList.remove(i);
                    aliasRedoList.remove(i);
                    extendedOuter.remove(i);
                    extendedOuterLevel.remove(i);
                    continue;
                } else {
                    tableOuterLevel = ((Integer) extendedOuterLevel
                            .get(i)).intValue();
                ...
                for (int k = 0; k < joinCondList.size(); k++) {
                    ...
                    if (joinStr.indexOf(" " + quotedTableName + ".") > 0
                        ...
                        for (int m = 0; m < tableRedoList.size(); m++) {
                            String testTable = IdentifierHandler.quote(
                                    tableRedoList.get(m));
                            ...
                            String testAlias = IdentifierHandler.quote(aliasRedoList.get(m));
                ...
                if (sOn.length() > 0 || joinCondList.size() == 0) {
                    ...
                    lastOuterLevel = tableOuterLevel;
                }
            }
        }
    ...
    private void addOuterIdColumns(QueryPlan aQueryPlan) {
        ...
        for (String sourceTableName : extendedTableList) {

            SysTable aSysTable = database.getSysTable(sourceTableName);
            ...
            if (!aSysTable.isTrueRowID()) {
                ...
                for (SysColumn aSysColumn : columnList) {
                    ...
                    aColumn = new Projection(
                            IdentifierHandler.quote(extendedTableList.get(0))
    ...
    public String addOid(QueryPlan aQueryPlan) {
        ...
        if (joinTableName == null) {
            ...
            // identifier
            aColumn = new Projection(IdentifierHandler.quote(extendedTableList.get(0))
                    + ".OID");
        } else {
...
//        selectColumnMap.put(aColumn.projectString, aColumn);
    ...
    protected Leaf createDerivedOuterLeaf () {
        ...
        outerLeaf.extendedTableList.addAll(extendedTableList);
        outerLeaf.extendedAliasList.addAll(extendedAliasList);
        outerLeaf.extendedOuter.addAll(extendedOuter);
        outerLeaf.extendedOuterLevel.addAll(extendedOuterLevel);
