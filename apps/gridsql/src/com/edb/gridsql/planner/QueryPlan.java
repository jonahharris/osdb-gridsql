...
public class QueryPlan
{
    ...
    private void createPlanSegmentFromTreeUnion(QueryTree aQueryTree,
        ...
        if (this != this.topQueryPlan) {
            ...
            while (it.hasNext()) {
                SqlExpression aSqlExpression = it.next();
                aLeaf.appendProjection(aSqlExpression.getAlias(), 
                		aSqlExpression.getAlias(), 
    ...
    private void createPlanSegmentFromTreeDetail(QueryTree aQueryTree,
        ...
        try {
            ...
            if (aQueryTree.isContainsAggregates()
                    ...
                    && !aQueryTree.usesSingleDBNode(database)
    ...
    private void createRelationSubplans(QueryTree aQueryTree, int unionDepth)
        ...
        for (int i = 0; i < aQueryTree.getRelationSubqueryList().size(); i++) {
            logger.debug(" - relationSubqueryList - ");
            topQueryPlan.useAllNodes = true;

            RelationNode subTreeNode = aQueryTree.getRelationSubqueryList().get(i);
    ...
    private void createPlanStep(QueryTree aQueryTree, QueryNode aQueryNode)
        ...
        try {
            ...
            if (aQueryNode.getParent() == null
                    && (!aQueryTree.isContainsAggregates()
                            || aQueryTree.isPartitionedGroupBy() || aQueryTree.usesSingleDBNode(database))) {
                Leaf lastLeaf = aLeaf;
            ...
            // what there current temp table is.
            // (except for if we are the joining node right below a tree-down
            // node)
            if (!(aQueryNode.getParent() != null && aQueryNode.getParent().isTreeDownJoin() && aQueryNode.getParent().getLeftNode() == aQueryNode)) {
    ...
    private Leaf getStepToProcess (QueryNode aQueryNode, boolean isDistinct) {
        ...
        if (aQueryNode.isTreeDownJoin()) {
            ...
            aLeaf.extendedTableList.add(prevLeaf.getTargetTableName());
            aLeaf.extendedAliasList.add(prevLeaf.getTargetTableName());
            aLeaf.extendedOuterLevel.add(Integer.valueOf(
                    aQueryNode.getRightNode().getOuterLevel()));
            aLeaf.extendedOuter.add(Boolean.FALSE);
    ...
    private void checkForCartesianProduct (QueryNode aQueryNode, Leaf aLeaf) {
        ...
        if (!aLeaf.hasProjections()
            ...
            anET.type = com.edb.gridsql.parser.ExpressionType.CHAR_TYPE;
            anET.length = 1;
    ...
    private void determineExecutionNodes (Leaf aLeaf, Leaf extraLeaf) {
        ...
        if (!aLeaf.isCombineOnMain()) {
            ...
            for (int i = 0; i < aLeaf.queryNodeList.size(); i++) {
                DBNode dbNode = (DBNode) aLeaf.queryNodeList.get(i);
    ...
    private boolean needToGenerateSerialForOuter(Leaf prevLeaf) {
        ...
        if (prevLeaf.extendedTableList.size() <= 1) {
            try {
    ...
    private boolean isSpecialOuterAggregateCase(QueryTree aQueryTree) {
        ...
        if (aQueryTree.getQueryNodeTable().size() == 3
            ...
            for (int i = 0; i < aQueryTree.getProjectionList().size(); i++) {
                SqlExpression aSqlExpression = (SqlExpression) aQueryTree.getProjectionList().get(i);
    ...
    private void handleOuter(Leaf aLeaf, int prevOuterLevel,
        ...
        for (String baseTable : aLeaf.extendedAliasList) {
            ...
            if (groupPos > 0) {
                ...
                groupString = groupString.replaceAll("NULL,", "");
    ...
    public String replaceColumnsBasedOnTable(String inputString,
        ...
        for (int pos = targetString.toUpperCase().indexOf(
                replaceTable.toUpperCase()); pos >= 0; lastPos = pos + 1, pos = targetString.toUpperCase().indexOf(
                replaceTable.toUpperCase(), pos + 1)) {
            if (pos > 0) {
    ...
    private void createPlanStepCorrelated(QueryNode aQueryNode, Leaf aLeaf)
        ...
        try {
            ...
            subTreeNode = (QueryNode) aQueryNode.getRightNode();
    ...
    private void createPlanStepDoConditions(QueryNode aQueryNode, Leaf aLeaf,
        ...
        try {
            ...
            List newConds = new ArrayList();

            for (int i = 0; i < aQueryNode.getConditionList().size(); i++) {
                aQueryCondition = (QueryCondition) aQueryNode.getConditionList().get(i);
            ...
            for (int i = 0; i < aQueryNode.getConditionList().size(); i++) {
                ...
                if (aQueryCondition.isJoin()
                    ...
                    try {
                        node1 = (RelationNode) aQueryCondition.getRelationNodeList().get(0);
                        ...
                        if (aQueryCondition.getRelationNodeList().size() != 2
                            ...
                            node2 = (RelationNode) aQueryCondition.getRelationNodeList().get(1);
                            ...
                            if (aQueryCondition.isSimpleTableJoin()
                                    && aQueryCondition.getOperator() == "="
                                    && aQueryCondition.getLeftCond().getExpr().getMappedExpression() == null
                                    && !aQueryNode.isTreeDownJoin()) {
                                // Now check if left side is partitioned column
                                AttributeColumn anAC = aQueryCondition.getLeftCond().getExpr().getColumn();

                                SysTable sysTab = null;

                                try {
                                    sysTab = anAC.getSysTable(database);
                                } catch (Exception e) {
                                    // ignore, we may be looking up a view.
                                }

                                if (aQueryNode.getRightNode().subtreeContains(anAC.relationNode)
                                        && sysTab != null
                                        && sysTab.getPartitionScheme() == SysTable.PTYPE_HASH
                                        && sysTab.getPartitionColumn().equalsIgnoreCase(
                                                anAC.columnName)) {
                                    // hash based on right side.
                                    aLeaf.setHashInfo(
                                            anAC.getTableName(),
                                            aQueryCondition.getRightCond().getExpr().getExprString());
                                } else {
                                    // Now check if right side is partitioned
                                    // column
                                    anAC = aQueryCondition.getRightCond().getExpr().getColumn();
                                    ...
                                    if (aQueryNode.getRightNode().subtreeContains(anAC.relationNode)
                                        ...
                                        aLeaf.setHashInfo(
                                            anAC.getTableName(),
                                            aQueryCondition.getLeftCond().getExpr().getExprString());
                                    }
                                }
                            }
                            ...
                            if (node1.getOuterLevel() != node2.getOuterLevel()
                                ...
                                if (aQueryCondition.getLeftCond().getExpr() != null
                                        && aQueryCondition.getRightCond().getExpr() != null) {
                                    if (aQueryCondition.getLeftCond().getExpr().contains(node1)
                                            && aQueryCondition.getRightCond().getExpr().contains(node2)) {
                                        ...
                                        aLeaf.outerExprList.add(aQueryCondition.getLeftCond().getExpr());
                                        aLeaf.innerExprList.add(aQueryCondition.getRightCond().getExpr());
                                        savedOk = true;

                                    } else if (aQueryCondition.getLeftCond().getExpr().contains(node2)
                                            && aQueryCondition.getRightCond().getExpr().contains(node1)) {
                                        if (aLeaf.outerExprList == null) {
                                            aLeaf.outerExprList = new ArrayList<SqlExpression>();
                                            aLeaf.innerExprList = new ArrayList<SqlExpression>();
                                        }
                                        aLeaf.outerExprList.add(aQueryCondition.getRightCond().getExpr());
                                        aLeaf.innerExprList.add(aQueryCondition.getLeftCond().getExpr());
                                        savedOk = true;
                                    }
                                }
            ...
            if (!aQueryNode.isPreserveSubtree()) {
                for (int i = 0; i < joinCondGroup.size(); i++) {
                    aQueryCondition = (QueryCondition) joinCondGroup.get(i);
            ...
            if (levelStatus == 3) {
                ...
                aLeaf2.extendedTableList.add(aLeaf.getTargetTableName());
                aLeaf2.extendedAliasList.add(aLeaf.getTargetTableName());
                aLeaf2.extendedOuter.add(Boolean.TRUE);
                // Given new outer scheme, force this to be a
                // higher outerlevel, add +1
                aLeaf2.extendedOuterLevel.add(new Integer(
                ...
                for (int i = 0; i < joinCondGroup2.size(); i++) {
                    aQueryCondition = (QueryCondition) joinCondGroup2.get(i);
        ...
        try {
            ...
            if (aQueryNode.getParent() == null
                ...
                for (SqlExpression aSqlExpression : aQueryNode.getProjectionList()) {
                    ...
                    // mds- need to check for underlying table and substitute
    ...
    private void createPlanStepDoGroupByList(QueryTree aQueryTree,
        ...
        try {
            ...
            for (int i = 0; i < aQueryNode.getGroupByList().size(); i++) {
                aSqlExpression = (SqlExpression) aQueryNode.getGroupByList().get(i);
            ...
            for (QueryCondition aQueryCondition : aQueryNode.getRelationNode().getSubqueryTree().getConditionList()) {
                for (RelationNode relCheckNode : aQueryCondition.getRelationNodeList()) {
                    ...
                    if (relCheckNode == correlatedNode.getRelationNode()) {
                        ...
                        QueryCondition parentCondition = aQueryCondition.getLeftCond(); 
                        QueryCondition childCondition = aQueryCondition.getRightCond(); 
                        testExpr = parentCondition.getSqlExpressionExclusiveFromTable(subqueryFinalTempName);
                        if (testExpr == null) {
                        	// try swap parent and child
                        	parentCondition = aQueryCondition.getRightCond();
                        	childCondition = aQueryCondition.getLeftCond();
                            testExpr = parentCondition.getSqlExpressionExclusiveFromTable(subqueryFinalTempName);
                        ...
                        if (testExpr != null) {
                            ...
                            // Now check for child.
                            testExpr = childCondition.getSqlExpressionExclusiveFromTable(aQueryNode.getRelationNode().getTableName());

                            if (testExpr != null) {
                                aLeaf.addCorrelatedColumn(testExpr);

                                // only set if we have not yet set
                                if (!aLeaf.isCorrelatedHashable()) {
                                    aLeaf.setCorrelatedHashable(true);
                                    aLeaf.correlatedChildHashableExpression = testExpr;
        ...
        try {
                            ...
                            if (aQC.getRightCond().getCondType() == QueryCondition.QC_SQLEXPR
                                ...
                                if (aQC.getRightCond().getCondString().equalsIgnoreCase(uncorRelationNode.getParentNoncorExpr().getExprString())) {
                                    ...
                                    if (aQC.getLeftCond().getCondType() == QueryCondition.QC_SQLEXPR
                                        ...
                                        SysTable sysTab = anAC.getSysTable(database);

                                        if (sysTab.getPartitionScheme() == SysTable.PTYPE_HASH
                                                && sysTab.getPartitionColumn().equalsIgnoreCase(
                                                        anAC.columnName)) {
            ...
            if (aQueryNode.getNodeType() == QueryNode.JOIN) {
                ...
                if (aQueryNode.isSubtreeOuter()) {
                    ...
                    aLeaf.extendedOuter.set(0, Boolean.TRUE);
                }
            } else {
                ...
                if (aQueryNode.getRelationNode().getNodeType() != RelationNode.SUBQUERY_NONCORRELATED
                    ...
                    aLeaf.extendedOuterLevel.add(Integer.valueOf(
                            aQueryNode.getOuterLevel()));

                    if (aQueryNode.isSubtreeOuter()) {
                        aLeaf.extendedOuter.add(Boolean.TRUE);
                    } else {
                        aLeaf.extendedOuter.add(Boolean.FALSE);
                    }
                    ...
                    aLeaf.extendedTableList.add(aQueryNode.getRelationNode().getTableName());
                    if (aQueryNode.getRelationNode().getAlias() != null
                            && aQueryNode.getRelationNode().getAlias().length() > 0) {
                        aLeaf.extendedAliasList.add(aQueryNode.getRelationNode().getAlias());
                    } else {
                        aLeaf.extendedAliasList.add(aQueryNode.getRelationNode().getTableName());
                    }
    ...
    private void evaluateConstantExpressions(QueryCondition aQueryCondition) {
        ...
        List<QueryCondition> sqlExpressionList = aQueryCondition.getNodes(aQueryCondition,
    ...
    private void adjustFinalProjections(Leaf aLeaf, List<SqlExpression> projectionList,
        ...
        try {
            ...
            for (int i = 0; i < projectionList.size(); i++) {
                aSqlExpression = (SqlExpression) projectionList.get(i);
    ...
    private void adjustNonAggOrderBy(Leaf aLeaf, List<OrderByElement> orderByList) {
        ...
        try {
            ...
            for (int i = 0; i < orderByList.size(); i++) {
                anOBE = (OrderByElement) orderByList.get(i);
    ...
    private void createFinalPlanStep(QueryTree aQueryTree) {
        ...
        try {
            ...
            aLeaf.extendedTableList.add(aLeaf.getTableName());
            aLeaf.extendedAliasList.add(aLeaf.getTableName());
            aLeaf.extendedOuter.add(Boolean.FALSE);
            aLeaf.extendedOuterLevel.add(Integer.valueOf(1));
            ...
            for (int i = 0; i < aQueryTree.getHavingList().size(); i++) {
                ...
                aQueryCondition = (QueryCondition) aQueryTree.getHavingList().get(i);
    ...
    /**
     * Replaces mapped expressions we renamed for SELECT, ORDER BY, GROUP BY
     *
     * @param aSqlExpression
     *            the expression to process
     */
    private void replaceMappedColumns(SqlExpression aSqlExpression) {
        final String method = "replaceMappedColumns";
        logger.entering(method);

        try {
            replaceColumnExpression(aSqlExpression);
    ...
    private void addHavingConditions(QueryTree aQueryTree, Leaf aLeaf) {
        ...
        try {
            for (int i = 0; i < aQueryTree.getHavingList().size(); i++) {
                ...
                QueryCondition aQueryCondition = (QueryCondition) aQueryTree.getHavingList().get(i);
    ...
    private void updateAggProjections(QueryTree aQueryTree, Leaf aLeaf) {
        ...
        try {
            ...
            for (int i = 0; i < aQueryTree.getProjectionList().size(); i++) {
                aSqlExpression = (SqlExpression) aQueryTree.getProjectionList().get(i);
                ...
                if (!aSqlExpression.containsAggregates()) {
                    ...
                    Leaf.Projection aColumn = aLeaf.new Projection(
                            IdentifierHandler.quote(finalSE.getExprString()));
                    ...
                    if (!useSlowAggregation) {
                        for (SqlExpression deferredExpr : newSqlExpression.getDeferredExpressions()) {
                            ...
                            SqlExpression aSE = ((SqlExpression) deferredExpr.getFunctionParams().get(0)).copy();
            ...
            for (int i = 0; i < aQueryTree.getOrderByList().size(); i++) {
                OrderByElement anOBE = (OrderByElement) aQueryTree.getOrderByList().get(i);
            ...
            for (int i = 0; i < aQueryTree.getHavingList().size(); i++) {
                QueryCondition aQC = (QueryCondition) aQueryTree.getHavingList().get(i);
            ...
            if (aSqlExpression.getExprType() == SqlExpression.SQLEX_OPERATOR_EXPRESSION 
                ...
                // avg becomes SUM (SUM at nodes) / SUM (COUNT at nodes)
                    ...
                    for (int i = 0; i < aSqlExpression.getFunctionParams().size(); i++) {
                        SqlExpression paramExpression = (SqlExpression) aSqlExpression.getFunctionParams().get(i);
    ...
    private void handleProjectionAddStepDeferred(SqlExpression aSqlExpression,
        ...
        SqlExpression aNewSE = (SqlExpression) aSqlExpression.getFunctionParams().get(0);
    ...
    private void makeAvgExpression(SqlExpression aSqlExpression,
        ...
        aNewSE.setAlias(newColumnName);
        aNewSE.setTempExpr(true);
        aNewSE.setExprDataType(FunctionAnalysis.analyzeSumParameter(
        ...
        aNewSE.setAlias(newColumnName2);
        aNewSE.setTempExpr(true);
        aNewSE.setExprDataType(FunctionAnalysis.analyzeCountParameter(aSqlExpression));
