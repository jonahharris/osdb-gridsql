    ...
    public static String getJavaTypeDesc(int sqlType, int length, int precision, int scale, boolean withTimeZone) {
        ...
        switch (sqlType) {
            case Types.TIME:
                if(length > 0) {
                    if(withTimeZone) {
                        typeString = TypeConstants.TIMEWITHLENGTH_WITH_TIMEZONE_TEMPLATE;
                    } else {
                        typeString = TypeConstants.TIMEWITHLENGTH_WITHOUT_TIMEZONE_TEMPLATE;
                    }
                } else {
                    if(withTimeZone) {
                        typeString = TypeConstants.TIMEWITHOUTLENGTH_WITH_TIMEZONE_TEMPLATE;
                    } else {
                        typeString = TypeConstants.TIMEWITHOUTLENGTH_WITHOUT_TIMEZONE_TEMPLATE;
                    }
                }
            ...
            case Types.TIMESTAMP:
                if(length > 0) {
                    if(withTimeZone) {
                        typeString = TypeConstants.TIMESTAMPWITHLENGTH_WITH_TIMEZONE_TEMPLATE;
                    } else {
                        typeString = TypeConstants.TIMESTAMPWITHLENGTH_WITHOUT_TIMEZONE_TEMPLATE;
                    }
                } else {
                    if(withTimeZone) {
                        typeString = TypeConstants.TIMESTAMPWITHOUTLENGTH_WITH_TIMEZONE_TEMPLATE;
                    } else {
                        typeString = TypeConstants.TIMESTAMPWITHOUTLENGTH_WITHOUT_TIMEZONE_TEMPLATE;
                    }
                }
                return ParseCmdLine.substitute(typeString, map);
            default:
