...
public class XDBSessionContext implements MessageTypes, ITransaction {
    ...
    /**
     * @throws com.edb.gridsql.exception.XDBSecurityException
     */
    ...
    public ExecutionResult login(String password) throws XDBSecurityException,
            XDBServerException {

        if (userName == null) {
            throw new XDBServerException("User name is not specified");
        }
        SysLogin login = MetaData.getMetaData().getSysLogin(userName);
     ...
     */
    public ExecutionResult login(String dbName, String userName,
            String password, String mode) throws XDBSecurityException,
            XDBServerException {
     ...
     * @see com.edb.gridsql.engine.BatchInsertGroup
     */
     ...
     * @param execRequest
     */
    public void kill(ExecutableRequest execRequest) {
        boolean interrupt = false;
        synchronized (this) {
            execRequest.cancel();
            interrupt = executables.getFirst() == execRequest;
            executables.remove(execRequest);
            notifyAll();
        }
        if (interrupt) {
            synchronized (executorAccess) {
     ...
     * @throws XDBSecurityException
     * @see com.edb.gridsql.metadata.NodeDBConnectionInfo
     */
