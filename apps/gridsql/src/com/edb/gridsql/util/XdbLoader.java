...
public class XdbLoader {
    ...
    public static void main(String[] args) {
        try {
            ...
            filter = new LoaderFilter(new BufferedInputStream(input), 1024,
                    commitInterval);
            ...
            String outputDirName = ParseArgs.getStrArg(m, "-y");
            if (outputDirName != null) {
                File aFile = new File(outputDirName);
                if (aFile.isDirectory()) {
                    outputDir = aFile;
                }
                filter.keepOriginalFormat = keepOriginalFormat;
            }
            if (commitInterval > 0 && outputDir == null) {
                throw new Exception(
                        "Output directory for rejected chunks must be specified if commit interval is limited.");
            }
            ...
            if (badFileName != null) {
                filter.badLines = new FileOutputStream(badFileName);
                Map<String, TableColumnDescription> columnDescriptions = new LinkedHashMap<String, TableColumnDescription>();
                try {
                    ResultSet rs = oConn.getMetaData().getColumns("", "",
                            tableName, null);
                    try {
                        while (rs.next()) {
                            String colName = rs.getString("COLUMN_NAME");
                            TableColumnDescription colDesc = new TableColumnDescription();
                            colDesc.name = colName;
                            boolean nullable = rs.getInt("NULLABLE") != DatabaseMetaData.attributeNoNulls
                                || forceNotNulls != null && forceNotNulls.contains(colName);
                            colDesc.nullable = nullable;
                            columnDescriptions.put(colName, colDesc);
                        }
                    } finally {
                        rs.close();
                    }
                    if (columnDescriptions.isEmpty()) {
                        System.out
                                .println("WARNING: Table info is requested from server, but empty response is returned");
                    } else {
                        if (columns == null) {
                            filter.columnDescriptions = columnDescriptions
                                    .values()
                                    .toArray(
                                            new TableColumnDescription[columnDescriptions
                                                    .size()]);
                        } else {
                            List<TableColumnDescription> referencedDescs = new LinkedList<TableColumnDescription>();
                            for (String column : columns) {
                                int pos = 0;
                                int comma = 0;
                                while ((comma = column.indexOf(",", pos)) != -1) {
                                    String colName = column.substring(pos, comma);
                                    TableColumnDescription colDesc = columnDescriptions
                                            .get(colName);
                                    referencedDescs.add(colDesc);
                                    pos = comma + 1;
                                }
                                String colName = column.substring(pos);
                                TableColumnDescription colDesc = columnDescriptions
                                        .get(colName);
                                referencedDescs.add(colDesc);
                            }
                            filter.columnDescriptions = referencedDescs
                                    .toArray(new TableColumnDescription[referencedDescs
                                            .size()]);
                        }
                    }
        ...
        try {
                ...
                try {
                    ...
                    if (commitInterval > 1) {
                        autoretry(copyManager, filter.lastChunk,
                                filter.originalChunk, 0, filter.chunkPos, se
                                        .getMessage());
                    } else {
                        throw se;
                    }
            ...
            System.out.println(filter.lineCount - filter.badLineCount
                    + " input rows have been sent to Server in "
                    + totalChunkCount + " chunks");
            if (filter.badLineCount > 0) {
                System.out
                        .println(filter.badLineCount
                                + " rows do not match target table's layout and have been written to "
                                + badFileName);
            }
            if (badChunkCount > 0) {
    ...
    private static void autoretry(CopyManager copyManager, byte[][] chunk,
            byte[][] originalChunk, int pos, int length, String reason)
            throws IOException, SQLException {
        if (autoReduceRate < 2 || length <= minCommitInterval) {
            badChunkCount++;
            badLinesCount += length;
            
            // Construct the rejected filename of the form
            // <dbname>_<tablename>_<internalId>.tbl
            String fileName = oConn.getCatalog() + "_" + 
                    tableName + "_" + 
                    totalChunkCount + ".tbl";
            File rejected = new File(outputDir, fileName);
            FileOutputStream out = new FileOutputStream(rejected);
            ...
            for (int i = pos; i < pos + length; i++) {
                out.write(keepOriginalFormat ? originalChunk[i] : chunk[i]);
            }
            out.close();
        } else {
            ...
            for (int i = pos; i < pos + length; i += reducedSize) {
                ...
                try {
                    ...
                    autoretry(copyManager, chunk, originalChunk, i, size, se
                            .getMessage());
                }
            }
        }
    ...
    private static class LoaderFilter extends FilterInputStream {
        ...
        LoaderFilter(InputStream in, int bufferSize, int chunkSize) {
            super(in);
            outBuffer = new byte[bufferSize < 256 ? 256 : bufferSize];
