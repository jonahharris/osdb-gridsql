...
public class Util implements MessageTypes {
    ...
    public static Connection connect(Map<String, List<String>> parameters,
        ...
        if (url == null) {
            ...
            url = "jdbc:edb://" + host + ":" + port + "/"
                    + (adminMode ? Props.XDB_ADMIN_DATABASE : database);
            /*
             * No connection options for edb driver (?) String argDelimiter =
             * "?"; List<String> options = parameters.get("-o"); if (options !=
             * null) { for (String option : options) { url += argDelimiter +
             * option; argDelimiter = "&"; } }
             */
        }
