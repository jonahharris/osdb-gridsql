...
public class SqlInsertTable extends SqlModifyTable {
    ...
    public Object visit(InsertTable n, Object argu) {
        ...
        if (n.f4.which == 0) {
            ...
            addProjColsIfFoundNullColumn(aQueryTree, getColumnsInvolved().size());
        }
        return _ret;
    }
     ...
     *
     */
    void addProjColsIfFoundNullColumn(QueryTree aQueryTree, int maxCol) {
        List<SqlExpression> projList = aQueryTree.getProjectionList();
        if(projList.size() == 1 &&
                projList.get(0).getExprString() == null) {
            if (maxCol > 1) {
                SqlExpression aSE = projList.get(0);
                projList.clear();
                for ( int index = 0; index < maxCol; index++ ) {
                    projList.add(aSE.copy());
                }
            }
        } else if (maxCol == 0) {
