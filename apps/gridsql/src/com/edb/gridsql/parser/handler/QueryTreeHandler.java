...
import com.edb.gridsql.optimizer.QueryCondition.CompositeCondition;
...
import com.edb.gridsql.parser.core.CSQLParserConstants;
 ...
 */
public class QueryTreeHandler extends ObjectDepthFirst {
    ...
    public QueryTreeHandler(Command commandToExecute) {
        ...
        database = MetaData.getMetaData().getSysDatabase(
        ...
        for (SqlExpression sqlExpr : vProjList) {
            ...
            if (sqlExpr.getExprType() == SqlExpression.SQLEX_COLUMN) {
                if (sqlExpr.getColumn().getTableName() == null
                        ...
                        && sqlExpr.getColumn().getTableAlias().equals("") == false) {
                    vSqlExpression.add(sqlExpr);
        ...
        for (SqlExpression aSqlExpression : vSqlExpression) {
            ...
            if (aSqlExpression.getExprString().equals("*")) {
                ...
                for (RelationNode qn : vRelationNodeList) {
                    ...
                    vExpandedExpr.addAll(populateProjectionList(qn, database));
                }
     ...
     */
        ...
        if (qn.getNodeType() == RelationNode.TABLE) {
            ...
            // of the Subqery
            for (SqlExpression aSqlExpression : qn.getSubqueryTree().getProjectionList()) {
                ...
                aSqlColExpression.getColumn().relationNode = qn;
                ...
                fillColumnExpression(qn, aSqlColExpression, aSqlExpression);
                // Finally add the SqlExpression to the projection list
                result.add(aSqlColExpression);
            }
        }
        return result;
    }
    ...
    /**
     * This function encapsulates the functionalty of handling the columnName
    ...
    public Object visit(SelectWithoutOrder n, Object argu) {
        ...
        if (n.f1.present()) {
            ...
            for (Iterator it = n.f1.nodes.iterator(); it.hasNext();) {
                ...
                NodeOptional nOp = (NodeOptional) noS.nodes.elementAt(1);
                NodeChoice ch = (NodeChoice) noS.nodes.elementAt(2);
    ...
    public Object visit(SelectWithoutOrderAndSet n, Object argu) {
        ...
        for (QueryCondition aFromQueryCondition : aQueryTree.getFromClauseConditions()) {
            processWhereCondition(aFromQueryCondition, aQueryTree,
                    commandToExecute.getaQueryTreeTracker(), database, true);
        }
        // Step 4.1
        /*
        ...
        checkAndFillTableNames(aQueryTree.getGroupByList(),
                ...
                GROUPBY, commandToExecute.getaQueryTreeTracker(), database);
        // Step 7
        ...
        if (aQueryTree.getHavingList().size() > 0) {
            ...
            checkAndFillQueryConditionColumns(
                    ...
                    aQueryTree.getRelationNodeList(), aQueryTree.getProjectionList(),
                    HAVING, commandToExecute.getaQueryTreeTracker(), database);
        }
         ...
         */
        FillAllExprDataTypes(aQueryTree, MetaData.getMetaData().getSysDatabase(
                commandToExecute.getDBName()));
    ...
    public static void ProcessWhereCondition(QueryCondition aQueryCondition,
            QueryTree aQueryTree, QueryTreeTracker aQueryTreeTracker,
            SysDatabase database) {
        processWhereCondition(aQueryCondition, aQueryTree, aQueryTreeTracker,
                database, false);
    }
    ...
    public static void processWhereCondition(QueryCondition aQueryCondition,
            QueryTree aQueryTree, QueryTreeTracker aQueryTreeTracker,
            SysDatabase database, boolean isFromCondition) {
        ...
        CAnalyzeAndCompleteColInfoAndNodeInfo colinfo = checkAndFillQueryConditionColumns(
                ...
                aQueryTree.getProjectionList(), CONDITION, aQueryTreeTracker,
                database);
        ...
        if (colinfo.orphanExpressionVector.size() > 0) {
            ...
            for (SqlExpression aColumnExpression : colinfo.orphanExpressionVector) {
                ...
                QueryCondition joinsWith = null;
                QueryCondition thisCond = null;
                ...
                if (aParentCondition.getLeftCond().getExpr()
                        .containsColumn(aColumnExpression.getColumn())) {
                    joinsWith = aParentCondition.getRightCond();
                    thisCond = aParentCondition.getLeftCond();
                } else {
                    joinsWith = aParentCondition.getLeftCond();
                    thisCond = aParentCondition.getRightCond();
                }
                // Now we will start to think about what we should do to
                // handle different expression types
                if (joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_COLUMN) {
                    joinsWith.getExpr().getColumn().relationNode.getProjectionList()
                            .add(joinsWith.getExpr());
                    aQueryTree.getHiddenProjectionList().add(joinsWith.getExpr());
                } else if (joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_OPERATOR_EXPRESSION
                        || joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_FUNCTION
                        || joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_CASE
                        || joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_SUBQUERY
                        || joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_CONDITION) {
                    // mds  this was not being done
                    // old comments in the code led me to believe that this
                    // should be done as well. Still, this could cause problems.
                    Iterator itColumns = SqlExpression.getNodes(
                            joinsWith.getExpr(), SqlExpression.SQLEX_COLUMN)
                            .iterator();

                    while (itColumns.hasNext()) {
                        SqlExpression aSqlEx = (SqlExpression) itColumns.next();
                        aSqlEx.getColumn().relationNode.getProjectionList().add(aSqlEx);
                        aQueryTree.getHiddenProjectionList().add(aSqlEx);
                    }
                } else if (joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_CONSTANT) {
                    // Dont do anything -- except mark the query condition

                } else if (joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_COLUMNLIST) {
                ...
                } else if (joinsWith.getExpr().getExprType() == SqlExpression.SQLEX_LIST) {
                    // Dont do any thing
                }
                ...
                if (thisCond.getExpr().getExprType() == SqlExpression.SQLEX_OPERATOR_EXPRESSION
                        || thisCond.getExpr().getExprType() == SqlExpression.SQLEX_FUNCTION
                        || thisCond.getExpr().getExprType() == SqlExpression.SQLEX_CASE
                        || thisCond.getExpr().getExprType() == SqlExpression.SQLEX_SUBQUERY
                        || thisCond.getExpr().getExprType() == SqlExpression.SQLEX_CONDITION) {
                    // Check to see if another table's columns appear on same
                    // side of expression
                    Iterator itColumns = SqlExpression.getNodes(thisCond.getExpr(),
                            SqlExpression.SQLEX_COLUMN).iterator();
         ...
         */
        Vector topLevelQueryConditions = new Vector();
        if (isFromCondition && aQueryTree.isOuterJoin()) {
            topLevelQueryConditions.add(aQueryCondition);
        } else {
            topLevelQueryConditions = getTopLevelQueryConditions(aQueryCondition);

        }
        /*
         * We also need to set the top level condition RelationNodelist
         */
        Enumeration topLevelCondEnumeration = topLevelQueryConditions
                .elements();

        Vector allSetConditions = new Vector();

        while (topLevelCondEnumeration.hasMoreElements()) {
            QueryCondition qc = (QueryCondition) topLevelCondEnumeration
                    .nextElement();
            /**
        ...
        topLevelQueryConditions.removeAll(allSetConditions);
        /*
         *
        ...
        if (aSegregateJoinhelper.nonjoinQConditionVector != null
            ...
            Vector simpleExprForQueryTree = getSimpleExprForQueryTree(simpleCondVector);
    ...
    public static void setBelongsToTree(QueryCondition aQueryCondition,
        ...
        Vector allQueryCondtions = QueryCondition.getNodes(aQueryCondition,
                QueryCondition.QC_ALL);

        Enumeration anAllConditions = allQueryCondtions.elements();

        while (anAllConditions.hasMoreElements()) {
            QueryCondition qc = (QueryCondition) anAllConditions.nextElement();
            qc.setParentQueryTree(aQueryTree);
            if (qc.getCondType() == QueryCondition.QC_SQLEXPR) {
                if (qc.getExpr().getExprType() == SqlExpression.SQLEX_SUBQUERY) {
                    qc.getExpr().getSubqueryTree().setParentQueryTree(aQueryTree);
                }
        ...
        for (QueryCondition exprCondition : QueryCondition.getNodes(
            ...
            QueryCondition parentQueryCondition = exprCondition.getParentQueryCondition();
            for (SqlExpression expr : SqlExpression.getNodes(
                ...
                expr.getColumn().setParentQueryCondition(parentQueryCondition);
            }
        }
    }
    ...
    public static void SetBelongsToTree(SqlExpression aSqlExpression,
        ...
        Vector aSqlExpressionVector = SqlExpression.getNodes(aSqlExpression,
                SqlExpression.SQLEX_ALL);
        Enumeration allSqlExpression = aSqlExpressionVector.elements();
        while (allSqlExpression.hasMoreElements()) {
            Object sqlexp = allSqlExpression.nextElement();
            SqlExpression aSqlExpr = (SqlExpression) sqlexp;
            if ((aSqlExpr.getExprType() & SqlExpression.SQLEX_SUBQUERY) > 0) {
                aSqlExpr.getSubqueryTree().setParentQueryTree(aTreeToSet);
    ...
    public static void FillAllExprDataTypes(QueryTree aQueryTree,
        ...
        for (SqlExpression aSqlExpression : aQueryTree.getProjectionList()) {
            aSqlExpression.setExprDataType(SqlExpression
                    .setExpressionResultType(aSqlExpression, database));
            if(aSqlExpression.getColumn() != null) {
         ...
         */
        for (SqlExpression aSqlExpression : aQueryTree.getGroupByList()) {
            aSqlExpression.setExprDataType(aSqlExpression
                    .setExpressionResultType(aSqlExpression, database));
        }
        /**
        ...
        for (QueryCondition fromCondition : aQueryTree.getFromClauseConditions()) {
            for (QueryCondition aQueryCondition : QueryCondition.getNodes(
                ...
                aSqlExpression.setExprDataType(aSqlExpression.setExpressionResultType(
                        aSqlExpression, database));
            }
        }
         ...
         */
        if (aQueryTree.getWhereRootCondition() != null) {
            if ((aQueryTree.getWhereRootCondition().getCondType() & QueryCondition.QC_COMPOSITE) == QueryCondition.QC_COMPOSITE
                    && aQueryTree.getWhereRootCondition().getACompositeClause()
                            .getCompareExpressionQueryCondition() != null) {
                CompositeCondition theCompositeClause = aQueryTree
                        .getWhereRootCondition().getACompositeClause();
                QueryCondition qc = theCompositeClause
                        .getCompareExpressionQueryCondition();
                List<QueryCondition> qcs = theCompositeClause.getExpressionConditionList();
                if (qc.getCondType() == QueryCondition.QC_SQLEXPR) {
                    int expType = 0;
                    if (qc.getExpr().getExprType() == SqlExpression.SQLEX_COLUMN) {
                        qc.getExpr().getColumn().columnType = qc.getExpr().getColumn()
                                .getColumnType(database);
                        expType = qc.getExpr().getColumn().columnType.type;
                    }
                    if (qc.getExpr().getExprDataType() != null) {
                        expType = qc.getExpr().getExprDataType().type;
                    }
                    if (expType == ExpressionType.DATE_TYPE
                            || expType == ExpressionType.TIME_TYPE
                            || expType == ExpressionType.TIMESTAMP_TYPE) {
                        for (Object element : qcs) {
                            QueryCondition qcl = (QueryCondition) element;
                            if (qcl.getCondType() == QueryCondition.QC_SQLEXPR) {
                                if (qcl.getExpr().getExprType() == SqlExpression.SQLEX_CONSTANT) {
                                    switch (expType) {
                                    case ExpressionType.TIME_TYPE:
                                        qcl.getExpr().setConstantValue(SqlExpression
                                                .normalizeTime(qcl.getExpr().getConstantValue()));
                                        break;
                                    case ExpressionType.TIMESTAMP_TYPE:
                                        qcl.getExpr().setConstantValue(SqlExpression
                                                .normalizeTimeStamp(qcl.getExpr().getConstantValue()));
                                        break;
                                    case ExpressionType.DATE_TYPE:
                                        qcl.getExpr().setConstantValue(SqlExpression
                                                .normalizeDate(qcl.getExpr().getConstantValue()));
                                        break;
                                    default:
                                        break;
                                    }
                                    qcl.getExpr().setExprDataType(qcl.getExpr()
                                            .setExpressionResultType(qcl.getExpr(),
                                                    database));
                                }
                    ...
                    qc.getExpr().setExprDataType(qc.getExpr().setExpressionResultType(
                            qc.getExpr(), database));
                }

            } else {
                Vector exprListCondition = QueryCondition.getNodes(aQueryTree
                        .getWhereRootCondition(), QueryCondition.QC_SQLEXPR);
                Enumeration aEnumeration = exprListCondition.elements();
                while (aEnumeration.hasMoreElements()) {

                    SqlExpression aSqlExpression = ((QueryCondition) aEnumeration
                            .nextElement()).getExpr();
                    if (aSqlExpression.getColumn() != null) {
    ...
    public static List<SqlExpression> checkAndFillTableNames(List<SqlExpression> checkList,
            ...
            QueryTreeTracker aQueryTreeTracker, SysDatabase database) {
        ...
        while (itCheckList.hasNext()) {
            CAnalyzeAndCompleteColInfoAndNodeInfo analysisInfo = analyzeAndCompleteColInfoAndNodeInfo(
                    itCheckList, nodeList, projList, mode, aQueryTreeTracker,
                    database);
    ...
    public static CAnalyzeAndCompleteColInfoAndNodeInfo analyzeAndCompleteColInfoAndNodeInfo(
            ...
            int mode, QueryTreeTracker aQueryTreeTracker, SysDatabase database) {
        ...
        while (itSQLExprList.hasNext()) {
            ...
            while (enColNodes.hasMoreElements()) {
                ...
                try {
                                ...
                                if (aSqlExpression.getExprType() == SqlExpression.SQLEX_COLUMN) {
                                    ...
                                    if (toMatchWith == null) {
                                        ...
                                        if (!toMatchWith.getTableAlis().equals(
                                            ...
                                            throw new XDBServerException(
                                                    ErrorMessageRepository.AMBIGIOUS_COLUMN_REF
                                                            + "( "
                                                            + colToCheck.columnName
                                                            + " )",
                                                    0,
                                                    ErrorMessageRepository.AMBIGIOUS_COLUMN_REF_CODE);
                                        }
                                    }
                                }
                            }
                    ...
                    columnInformation.orphanExpressionVector
                            .add(colNotFoundException.getColumnExpression());

                    try {
                        QueryTree aParentTree = aQueryTreeTracker
                                .getParentTree(colNotFoundException
                                        .getColumnExpression());
                        List<SqlExpression> vColVector = new ArrayList<SqlExpression>();
                        vColVector.add(colNotFoundException
                                .getColumnExpression());
                        analyzeAndCompleteColInfoAndNodeInfo(vColVector
                                .iterator(), aParentTree.getRelationNodeList(),
                                aParentTree.getProjectionList(), mode,
                        ...
                        aOrphanColumnExpression.setExpressionResultType(
                                aOrphanColumnExpression, database);
                    } catch (NoParentTreeException e) {
    ...
    protected static RelationNode getTableRelationNode(
        ...
        for (RelationNode qTableNode : listOfRelationNodes) {
            ...
            if (qTableNode.getNodeType() == RelationNode.TABLE) {
                // Debug statment
                tableListChecked += "Alias" + qTableNode.getAlias() + " Name "
                        + qTableNode.getTableName();
    ...
    public static CAnalyzeAndCompleteColInfoAndNodeInfo checkAndFillQueryConditionColumns(
        ...
        CAnalyzeAndCompleteColInfoAndNodeInfo colInfo = analyzeAndCompleteColInfoAndNodeInfo(
                ...
                mode, aQueryTreeTracker, database);
    ...
    public static boolean isJoinQueryCondition(QueryCondition aQueryCondition)
        ...
        if (aQueryCondition.getCondType() == QueryCondition.QC_COMPOSITE) {
            if (aQueryCondition.getACompositeClause().leadsToJoinCondition() == true) {
                return true;
            }
        }

        // Get all the conditions
        Vector queryExprCondVector = QueryCondition.getNodes(aQueryCondition,
                QueryCondition.QC_RELOP_COMPOSITE);
        // Pick out the Delete all the And and OR
        Enumeration queryExprEnumeration = queryExprCondVector.elements();
        // Iterate thru the result - the relation op will have > , = etc.
        while (queryExprEnumeration.hasMoreElements()) {
            QueryCondition queryCond = (QueryCondition) queryExprEnumeration
                    .nextElement();
    ...
    /**
     * This is a hellper class which allows us to pass ORCONDITIONS amd AND
     * Conditions separately
     *
     * The conditions are mutuallyexclusive of each other
     */
    ...
    /*
     * Case 1: There are no and or condition (single condition) Case 2: Only And
     * Conditions Case 3: Both And OR Condition Case 4: Only Or Conditions
     */
    public static Vector<QueryCondition> getTopLevelQueryConditions(QueryCondition rootCondition) {
        /*
         * 1. Get the QueryCondition nodes
         *
         * 2. Filter thoes node s which have an AND and which have an OR
         *
         * 3. Start moving through the first OR condition node and remove it
         * from the list along with all its children.
         *
         * 4. go on until u have no more OR nodes
         *
         * 5. In the end you will be left with a bunch of AND and OR nodes which
         * are your TOPLEVELNODES
         */

        // We have filtered nodes now in the helper Object
        ParamAndOrConditionHelper helper = getAndOrNodes(rootCondition);

        // This will contain the or condition - Which are present in this
        // particular query conditions
        Vector<QueryCondition> orConditions = helper.orConditionVector;

        // This will contian the and conditions - which are present in this
        // particular query condition
        Vector<QueryCondition> andConditions = helper.andConditionVector;

        /*
         * Incase we have no AND or OR conditions these vectors(orCondition &
         * andCondition) will remain empty There fore for case 1 we have to do
         * special handling for Case 2 3 and 4 we proceed below
         */

        // This variable is used later to hold Or Condition Vectors
        Vector<QueryCondition> globalOrCondVector = new Vector<QueryCondition>();

        /*
         * Incase we have case 2 Only And Conditions we wont have to loop
         * through the while Loop below
         *
         * But for Case 3 and 4
         *
         * For each Or Condition - we will again try to find all the AND child
         * nodes and OR child nodes and delete them .This will eventually leave
         * us with only the top level nodes.
         *
         */

        Vector<QueryCondition> localVector = new Vector<QueryCondition>();

        for (QueryCondition qcor : orConditions) {

            /*
             * Find the And and OR Condition - children of this OR Condition
             */
            /*
             * 2 Cases Arise here - Depending on if we encounter child first or
             * the parent.It is easy to maintain if the order in which we get
             * the nodes do not effect the logic
             *
             * a) Or Condition is a Parent of another Contion and comes before
             * the child b)Or Condition is a Parent of another Contion and comes
             * after the child.
             *
             * Incase we have case (b) Then the parent will remove the child and
             * we will have just the parent condition to deal with
             *
             * Incase it is case (a) localVector will contain the child and we
             * will not process it
             *
             */

            // Incase we have already processed this query condition
            // as it is below some other OR condition - We dont want
            // to process it again. Note Local Vector contains the children of
            // all the nodes which
            // are added to the globalOrCondVector
            if (!localVector.contains(qcor)) {
                ParamAndOrConditionHelper helper2 = getAndOrNodes(qcor);

                // And Remove the and Condition from the List andConditions --
                // which was
                // originally got from the root condition
                Vector<QueryCondition> vAndConditions = helper2.andConditionVector;

                andConditions.removeAll(vAndConditions);

                // We also need to remove any of the or conditions which lie
                // below and have been added. - but we need to be careful
                // that we are not processing the condition in the order that
                // the
                // larger query emcompasses the smaller query which is also
                // processed
                // first - Therefore we make a check with the localVector

                localVector.addAll(helper2.orConditionVector);
                globalOrCondVector.removeAll(helper2.orConditionVector);
                globalOrCondVector.add(qcor);
            }
        }
        // Once We are Here -- We have a list of Or conditions and a list of And
        // Conditions and the list of
        // or conditions which are not a child of any other OR condition in the
        // list or And Condition in the list

        /*
         * The following cases can arise - If there is no And condition or
         *
         * the top level condition is an OR
         *
         * We will have globalvector filled by the top level condition. : case 4
         *
         * What if we dont have any AND condition. case 1
         */

        // Now we should create a new vector and club what ever is left in the
        // or conditions and
        // the and conditions.
        Vector<QueryCondition> toplevelConditionVector = new Vector();

        // This handles the first case - when we dont have any or or and
        // conditions
        if (andConditions.size() <= 0 && orConditions.size() <= 0
                && globalOrCondVector.size() <= 0) {
            toplevelConditionVector.add(helper.singleQueryCondition);
        } else {
            // Where as the "Or" Conditions will be added as it is
            // We need to add the left and right conditions of the
            // "and" condition.

            for (QueryCondition andQc : andConditions) {
                /**
                 * We can have Composite Expressions or Regular Expressions
                 * passing through here and also Condition expressions
                 *
                 * As the logic goes - We must allow any expression which
                 * are below a AND expression and which is not and
                 * expression to be added. for eg
                 *
                 * and ((2 > 3) and (3> 4) ) and ((4 > 5) and (5>6))
                 *  | (2>3) and (3>4)and (4>5) and (5>6) |----------| and
                 * and | |--------| 2 > 3 3 > 4
                 */

                if ((andQc.getLeftCond().getCondType() & QueryCondition.QC_COMPOSITE) > 0
                        || !"AND".equalsIgnoreCase(andQc.getLeftCond().getOperator())) {
                    toplevelConditionVector.add(andQc.getLeftCond());
                }
                if ((andQc.getRightCond().getCondType() & QueryCondition.QC_COMPOSITE) > 0
                        || !"AND".equalsIgnoreCase(andQc.getRightCond().getOperator())) {
                    toplevelConditionVector.add(andQc.getRightCond());
                }
            }

            for (QueryCondition orQc : globalOrCondVector)
            {
                if (toplevelConditionVector.contains(orQc) == false) {
                    toplevelConditionVector.add(orQc);
                }
            }

            // Now we will add all the OR conditions which we have so patiently
            // collected
            // in globalOrCondVector
            for (QueryCondition gOrQc : globalOrCondVector) {
                if (toplevelConditionVector.contains(gOrQc) == false) {
                    toplevelConditionVector.add(gOrQc);
                }
            }
        }
        return toplevelConditionVector;
    }

    /*
     * Given any QueryCondition this function will help us seg. the and or
     * queryconditions
     */

    private static ParamAndOrConditionHelper getAndOrNodes(
            QueryCondition aQueryCondition) {
        Vector<QueryCondition> vQueryConditions = QueryCondition.getNodes(
                aQueryCondition,
                QueryCondition.QC_CONDITION);
        // Incase we have a single condition -- then we are not going to have
        // and or or
        // conditions.
        if (vQueryConditions.isEmpty()) {
            vQueryConditions = QueryCondition.getNodes(aQueryCondition,
                    QueryCondition.QC_RELOP_COMPOSITE);
        }
        if (vQueryConditions.isEmpty()) {
            vQueryConditions = new Vector<QueryCondition>(1);
            vQueryConditions.add(aQueryCondition);
        }

        ParamAndOrConditionHelper helper = new ParamAndOrConditionHelper();
        Vector<QueryCondition> orConditions = new Vector<QueryCondition>();
        Vector<QueryCondition> andConditions = new Vector<QueryCondition>();
        for (QueryCondition qc : vQueryConditions) {
            // If we have a operator set we are sure it will be either an AND or
            // an OR
            // as we have filtered on them. -
            // We however can also have composite if they are single
            // query conditions and operator as the statement .
            if ((qc.getCondType() & QueryCondition.QC_COMPOSITE) == 0) // Implying
                                                                    // that we
                                                                    // dont have
                                                                    // a
                                                                    // composite
                                                                    // Condition
            {
                if ("AND".equalsIgnoreCase(qc.getOperator())) {
                    andConditions.addElement(qc);
                } else if ("OR".equalsIgnoreCase(qc.getOperator())) {
                    orConditions.addElement(qc);
                } else {
                    helper.singleQueryCondition = qc;
                }
            } else {
                // If we have a composite condition
                helper.singleQueryCondition = qc;
            }
        }
        helper.orConditionVector = orConditions;
        helper.andConditionVector = andConditions;
    ...
    public static SegregateJoinHelper segregateIntoSimpleAndJoinConditions(
        ...
        // For each condition
        while (enCond.hasMoreElements()) {
            // extract the conditions from the enumeration
            QueryCondition qc = (QueryCondition) enCond.nextElement();
    ...
    public static void populateJoinExpressionCrossReference(
            Vector joinQConditionVector) throws ColumnNotFoundException {
        /*
         * The join vector must only contain the QueryCondition with joins
         */

        Enumeration enJoinVector = joinQConditionVector.elements();

        // For each Query Condition
        while (enJoinVector.hasMoreElements()) {

            QueryCondition qc = (QueryCondition) enJoinVector.nextElement();
            qc.setJoin(true);

            // Get the nodes with RELOP i.e. with the operator > , = etc.

            Vector qcRelop = QueryCondition.getNodes(qc,
                    QueryCondition.QC_RELOP_COMPOSITE);

            Enumeration enRelopNodes = qcRelop.elements();

            // Now for each of these Query Condition

            while (enRelopNodes.hasMoreElements()) {
                /* For each relop node get the Query Nodes Involved */
                QueryCondition qcRelopNode = (QueryCondition) enRelopNodes
                        .nextElement();
    ...
    public static Vector getSimpleExprForQueryTree(Vector simpleCondVector)
            throws ColumnNotFoundException {
        Enumeration QueryCondEnumeration = simpleCondVector.elements();
        Vector QueryCondVector = new Vector();
        while (QueryCondEnumeration.hasMoreElements()) {
            QueryCondition qc = (QueryCondition) QueryCondEnumeration
                    .nextElement();
    ...
    public static void addToNodesSimpleQueryCondition(
            Vector simpleQueryCondition, QueryTree aQueryTree)
            throws ColumnNotFoundException {
        Enumeration enQueryCondEnumeration = simpleQueryCondition.elements();

        while (enQueryCondEnumeration.hasMoreElements()) {
            QueryCondition qc = (QueryCondition) enQueryCondEnumeration
                    .nextElement();
            List<RelationNode> RelationNodeVector = getRelationNodes(qc);
     ...
     *
     */
    protected void manageFromSubQuery(QueryTree aQueryTree) {
            ...
            while (colVecEnumeration.hasMoreElements()) {
                ...
                if (sqlColumnExpr.getColumn().relationNode.getNodeType() == RelationNode.SUBQUERY_RELATION) {
                    try {
                        ...
                        if (aQueryTree.getRelationSubqueryList()
                                .contains(sqlColumnExpr.getColumn().relationNode) == false) {
                            aQueryTree.getRelationSubqueryList()
                                    .add(sqlColumnExpr.getColumn().relationNode);
                        }
