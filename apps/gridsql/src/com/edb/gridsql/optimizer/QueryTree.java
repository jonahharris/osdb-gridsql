...
public class QueryTree implements IRebuildString {
    ...
    private List<RelationNode> noncorSubqueryList; // of type RelationNode

    private List<RelationNode> correlatedSubqueryList; // of type RelationNode
     ...
     * {@link com.edb.gridsql.metadata.SyncCreateTable} to do this
     * @param client
     * @return
     * @throws Exception
     */
    ...
    public QueryTree copy() {
        ...
        newQueryTree.parentTree = this.parentTree;
    ...
    public RelationNode newRelationNode() {
        ...
        aRelationNode.setNodeId(++currentRelationNodeId);
        ...
        if (groupByList.size() > 0) {
            ...
            if (firstGroupExpr.getExprType() == SqlExpression.SQLEX_COLUMN) {
                ...
                if (groupTableNode != null) {
                    ...
                    // for the table.
                    groupSysTable = firstGroupExpr.getColumn().getSysTable(database);

                    if (groupSysTable.getPartitionScheme() == SysTable.PTYPE_HASH
                            && groupSysTable.getPartitionColumn()
                                    .compareToIgnoreCase(
                                            firstGroupExpr.getColumn().columnName) == 0) {
     ...
     * @return
     */
    public boolean isSingleTableLookup(SysDatabase database) {
        ...
        SysTable aSysTable = relNode.getSysTable(database);

        if (!aSysTable.isLookup()) {
            return false;
        }
     ...
     * @return
     */
    public boolean usesSingleDBNode(SysDatabase database) {
        if (this.rootNode.getRightNode() != null) {
            return false;
        }
        ...
        if (aSysTable.isLookup() || aSysTable.getNodeList().size() == 1) {
            return true;
        }
    ...
    public boolean containsOnlyLookups(SysDatabase database) {
        ...
        try {
            ...
            for (RelationNode aRelNode : getRelationNodeList()) {
                ...
                // check subtree type
                if (!aRelNode.getSysTable(database).isLookup()) {
                    return false;
                }
            }
