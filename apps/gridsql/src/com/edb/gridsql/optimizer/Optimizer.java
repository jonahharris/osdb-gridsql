 ...
 *
 * Notes:
 * Always evaluate non-correlated subqueries first for a relation
 * Try and evelauate correlated subqueries last, or near the end
 * Always perform join of parent-child partitions together,
 * which is determined early in query plan
 * Always perform joins with "lookup" tables together
 * This last rule should be revisited- these joins should be biased to be moved
 * to the end if possible, except if there are additional qualifying conditions
 *
 */
public class Optimizer {
     ...
     * and only one of the tables joins with a table on a lower level.
     */
    private static final int OUTER_PARTED_SAME_ONE = 2;

    /**
     * Each are on a different level,
     * separated by 1, and no other tables are in their respective outer groups.
     */
        ...
        try {
            ...
            for (RelationNode aRelNode : subTreeNode.getSubqueryTree().getRelationNodeList()) {
                ...
                if (aRelNode.getNodeType() == RelationNode.TABLE
                        && !aRelNode.getSysTable(database).isLookup()) {
                    if (++nonLookupCount >= 2) {
                        return false;
                    }
                }
            ...
            for (QueryCondition cond : subTreeNode.getSubqueryTree().getConditionList()) {
                if (!cond.isTwoRelationJoin()) {
                    ...
                    if (aRelationNode.isCorrelatedPlaceholder()) {
                        if (otherRelationNode.getSysTable(database).isLookup()) {
                            continue;
                        }
                    } else if (otherRelationNode.isCorrelatedPlaceholder()) {
                        if (aRelationNode.getSysTable(database).isLookup()) {
                            continue;
                        }
                    } else {
                        if (aRelationNode.getSysTable(database).isLookup()
                                && otherRelationNode.getSysTable(database)
                                .isLookup()) {
                            continue;
                        }
                    }
                    ...
                    if (!isCorrJoin) {
                        ...
                        partitionColumn = aRelationNode.getSysTable(database)
                        .getPartitionColumn();
                        ...
                        if (partitionColumn != null
                            ...
                            joinRelationNode = isCorrelatedPartitionedJoin (
                                    cond.getLeftCond(), cond.getRightCond(),
                            ...
                            joinRelationNode = isCorrelatedPartitionedJoin (
                                    cond.getRightCond(), cond.getLeftCond(),
                                    aRelationNode, partitionColumn);
                        }
                    }
                }
            ...
            for (RelationNode relNode :
                ...
                if (!relNode.getSysTable(database).isLookup()) {
     ...
     */
    private RelationNode isCorrelatedPartitionedJoin (QueryCondition condition1,
            QueryCondition condition2, RelationNode aRelationNode,
            String partitionColumn) {

        RelationNode joinRelationNode = null;

        if (condition1.getExpr().isColumn()) {
            AttributeColumn column1 = condition1.getExpr().getColumn();
            ...
            if (column1.getTableName().equalsIgnoreCase(
                    ...
                    && condition2.getExpr().isColumn()) {

                AttributeColumn column2 =  condition2.getExpr().getColumn();

                String otherPartitionColumn = column2.getSysTable(database)
                .getPartitionColumn();

                if (column2.columnName.equalsIgnoreCase(otherPartitionColumn)) {
                    // ok, we have a valid correlated
                    // join on partitioned columns
                    SysTable aSysTable = column1.getSysTable(database);
                    SysTable otherSysTable = column2.getSysTable(database);
    ...
    private boolean checkParentChildInCondition(QueryCondition aQC) {
        ...
        if (aQC.getLeftCond().getCondType() == QueryCondition.QC_SQLEXPR
            ...
            if (anAC.getSysTable(database).isPartitionedColumn(
                    anAC.columnName)) {
                ...
                if (aQC.getRightCond().getCondType() == QueryCondition.QC_SQLEXPR
                    ...
                    if (projAC.getSysTable(database).isPartitionedColumn(
                            projAC.columnName)) {
                        ...
                        if (!projAC.getSysTable(database).onSameNodes(
                                anAC.getSysTable(database))) {
                            return false;
                        }
                        ...
                        for (RelationNode subqueryRelNode :
                            ...
                            if (!subqueryRelNode.getSysTable(database)
                                    .isLookup()) {
                                return false;
                            }
                        }
    ...
    throws XDBServerException {
        ...
        try {
            ...
            // process relation subqueries first (FROM clause)
    ...
    /**
     * Checks to see if condition part is a column in the specified table
     * @param aQueryCondition
     * @param tableName
     * @return the AttributeColumn if it is a match, otherwise null
     */
    private AttributeColumn checkConditionMatch(QueryCondition aQueryCondition,
            String tableName) {

        AttributeColumn anAC = null;

        if (aQueryCondition.getExpr() != null
                && aQueryCondition.getExpr().isColumn()) {

            anAC = aQueryCondition.getExpr().getColumn();
        ...
        try {
            ...
            for (int i = 0; i < aQueryTree.getConditionList().size(); i++) {
                ...
                if (cond.isTwoRelationJoin()) {
                    ...
                    if (aRelationNode.getSysTable(database)
                            .getPartitionScheme() == SysTable.PTYPE_HASH
                            && otherRelationNode.getSysTable(database)
                            .getPartitionScheme() == SysTable.PTYPE_HASH) {
                        ...
                        if (cond.getRightCond().getExpr().isColumn()) {
                        ...
                        // Make sure they are partitioned on the same nodes
                        if (!aRelationNode.getSysTable(database).onSameNodes(
                                otherRelationNode.getSysTable(database))) {
                            continue;
                        }
                    ...
                    } else if (aRelationNode.getSysTable(database)
                            .getPartitionScheme() == SysTable.PTYPE_ONE
                            && otherRelationNode.getSysTable(database)
                            .getPartitionScheme() == SysTable.PTYPE_ONE) {
            ...
            Map<RelationNode,List> lookupMap = new HashMap<RelationNode, List>();
            ...
            for (int i = 0; i < aQueryTree.getConditionList().size(); i++) {
                ...
                if (cond.isTwoRelationJoin()) {
                    ...
                    if (aRelationNode.getSysTable(database).isLookup()
                        ...
                        if (outerType != OUTER_PARTED_NO
                                && !(outerType == OUTER_PARTED_DIFFERENT && aRelationNode
                                        ...
                                        .getOuterLevel())) {
                            List vConditions = lookupMap.get(aRelationNode);

                            if (vConditions == null) {
                                vConditions = new ArrayList();
                            }
                    ...
                    } else if (otherRelationNode.getSysTable(database).isLookup()
            ...
            for (RelationNode lookupRelationNode : lookupMap.keySet()) {
                List<LookupCandidate> vConditions = lookupMap.get(lookupRelationNode);

                // If there is only 1, just choose it
                if (vConditions.size() == 1) {
                    LookupCandidate aLookup = vConditions
                    .get(0);
                    ...
                    for (LookupCandidate aCandidate : vConditions) {
                        ...
                        SysTable sysTable = lookupRelationNode.getSysTable(database);

                        anAC = checkConditionMatch(
                                aCandidate.queryCondition.getLeftCond(), sysTable
                                .getTableName());

                        // check other part of condition
                        if (anAC == null) {
                            anAC = checkConditionMatch(
                                    aCandidate.queryCondition.getRightCond(),
    ...
    private boolean checkAndMergePartitionedColumns (
        ...
        if (column1.getTableName().equalsIgnoreCase(
            ...
            String otherPartitionColumn = otherRelationNode.getSysTable(database)
            .getPartitionColumn();
     ...
     * partitioning key. <br>
     * It should only allow them to be processed together under the following
     * scenarios:<br>
     * 1.) Both tables are on level 0.<br>
     * 2.) Both are on the same level, and only one of the tables joins with a
     * table on a lower level.<br>
     * 3.) Each are on a different level, separated by 1, and no other tables
     * are in their respective outer groups.<br>
     *
     * <br>
     * If none of these scenarios apply, these should not be processed together.
     *
     * @param node1
     *            RelationNode to check
     * @param node2
     *            other RelationNode to check
     * @param aQueryTree
     *            the QueryTree we are processing
     * @return the outer type - OUTER_PARTED_NO (0) - Do not process together,
     *         OUTER_PARTED_LEVEL_0 (1) - both are on level 0,
     *         OUTER_PARTED_SAME_ONE (2) - both are on same level,
     *         OUTER_PARTED_DIFFERENT (3) - these are on different levels
     */
     ...
     *
     * @param aQueryTree
     *            The QueryTree for which to get initial cost.
     */
    // -------------------------------------------------------------------------
        ...
        try {
            ...
            for (QueryNode currentNode : aQueryTree.getQueryNodeTable().values()) {
                ...
                if (currentNode.isTable())
                {
                    ...
                    SysTable aSysTable = currRelNode.getSysTable(database);
     ...
     *
     * @param aQueryTree
     *            The QueryTree we are building up
     * @param node1
     *            Existing QueryNode to join
     * @param node2
     *            Candidate QueryNode to join with
     * @param saveNode
     *            The new QueryNode joining node that joins the nodes
     */
     ...
     *
     * @param aQueryNode
     *            The QueryNode to mark
     * @param unusedQueryNodes
     *            The list of remaining unused QueryNodes
     */
     ...
     *
     * @param targetTree
     *            the QueryTree to search
     * @param targetLabel
     *            the QueryNode label to search for
     * @param aQueryCondition
     ...
     *            the column being used in the segment
     */
    private void addColumnProjection(QueryNode startNode, QueryNode endNode,
        ...
        try {
            ...
            aSqlExpression.setExprString(anAttributeColumn.getTableName() + "."
                    + anAttributeColumn.columnName);
     ...
     *            the expression containg the column to add
     */
    private void addColumnProjection(QueryNode startNode, QueryNode endNode,
