...
public class QueryCondition implements IRebuildString {
    ...
    /**
     * Check to see if the QueryCondition contains any expressions that are only
     * derived from the specified RelationNode. <br>
     * It returns a top level SqlExpression for it, so we can hash by it in
     * correlated joins. Note that we do not want to allow ORs or more
     * complicated expressions. In those cases, just return null.
     * @param checkTableName
     * @return
     */
    public SqlExpression getSqlExpressionExclusiveFromTable(
            String checkTableName) {
        if (this.condType != QueryCondition.QC_SQLEXPR) {
            return null;
        }

        if (this.expr.containsColumnsExclusiveFromTable(checkTableName)) {
            return expr;
        } else {
            return null;
        }
    ...
    public boolean isSimpleTableJoin() {
        ...
        if (getLeftCond() != null && getLeftCond().condType == QueryCondition.QC_SQLEXPR
                && getLeftCond().expr.getExprType() == SqlExpression.SQLEX_COLUMN) {
            if (rightCond != null
                    && rightCond.condType == QueryCondition.QC_SQLEXPR
                    && rightCond.expr.getExprType() == SqlExpression.SQLEX_COLUMN) {
                return true;
            }
        }
    ...
    public boolean isTwoRelationJoin() {    
        return (isJoin() 
                && getRelationNodeList().size() == 2 
                && getCondType() == QC_RELOP);
    }
    ...
    /**
     * Gets all the top level AND conditions We need to rebuild
     * relationNodeList.
     * @return
     */
    public Collection<QueryCondition> getAndedConditions() {
        Vector<QueryCondition> topConditions = new Vector<QueryCondition>();

        if (this.condType != QueryCondition.QC_CONDITION) {
            topConditions.add(this);
        } else {
            if (this.operator == "AND") {
                topConditions.addAll(getLeftCond().getAndedConditions());
                this.getLeftCond().setRelationNodes();
                this.getLeftCond().isJoin = true;
                topConditions.addAll(rightCond.getAndedConditions());
                this.rightCond.setRelationNodes();
                this.rightCond.isJoin = true;
            } else {
                // This will capture an or condition
                topConditions.add(this);
            }
        }
    ...
    private void setRelationNodes() {
        for (QueryCondition aQC : QueryCondition.getNodes(this,
                QueryCondition.QC_SQLEXPR)) {
            for (SqlExpression aSqlExpression : SqlExpression.getNodes(aQC.expr,
                    SqlExpression.SQLEX_COLUMN)) {
                AttributeColumn anAC = aSqlExpression.getColumn();

                if (!this.relationNodeList.contains(anAC.relationNode)) {
                    this.relationNodeList.add(anAC.relationNode);
                }
            }
        }
    }
    ...
    public Collection<DBNode> getPartitionedNode(XDBSessionContext client) {
        QueryCondition columnCondition;
        ...
        if (relationNodeList.size() != 1
                || !(condType == QueryCondition.QC_RELOP && operator == "=")) {
            return null;
        }
        ...
        if (getLeftCond() != null && getLeftCond().condType == QueryCondition.QC_SQLEXPR
                && getLeftCond().expr.getExprType() == SqlExpression.SQLEX_COLUMN
                && getLeftCond().expr.getMappedExpression() == null) {
            columnCondition = getLeftCond();
            valueCondition = rightCond;
        } else if (rightCond != null
                && rightCond.condType == QueryCondition.QC_SQLEXPR
                && rightCond.expr.getExprType() == SqlExpression.SQLEX_COLUMN
                && rightCond.expr.getMappedExpression() == null) {
            columnCondition = rightCond;
            valueCondition = getLeftCond();
        } else {
            return null;
        }

        // Now check if it is a partitioned column
        AttributeColumn anAC = columnCondition.expr.getColumn();
        SysTable sysTab = anAC.getSysTable(client.getSysDatabase());
        if (anAC.columnName.equalsIgnoreCase(sysTab.getPartitionColumn())) {
            // check if right side is an expression
            if (valueCondition.condType == QueryCondition.QC_SQLEXPR
                    && valueCondition.expr.getExprType() == SqlExpression.SQLEX_CONSTANT) {
                // Make sure value type is the same as column type
                if (columnCondition.expr.getExprDataType() == null) {
                    columnCondition.expr.setExprDataType(
                            columnCondition.expr.getColumn().getColumnType(
                                    client.getSysDatabase()));
                }
                valueCondition.expr.setExprDataType(columnCondition.expr.getExprDataType());
                Collection<DBNode> vNodes = sysTab.getNode(valueCondition.expr.getNormalizedValue());
    ...
    public SqlExpression getPartitionParameterExpression(SysDatabase database) {
        ...
        if (valueCondition.condType == QueryCondition.QC_SQLEXPR
                && valueCondition.expr.getExprType() != SqlExpression.SQLEX_PARAMETER) {
            return null;
        }
        ...
        if (anAC.columnName.equalsIgnoreCase(sysTab.getPartitionColumn())) {
            ...
            columnCondition.expr.setParamNumber(valueCondition.expr.getParamNumber());
            return columnCondition.expr;
        }
